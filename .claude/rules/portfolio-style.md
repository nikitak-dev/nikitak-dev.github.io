...
## Структурные лимиты (hard limits)

| Тип | Лимит | Действие при превышении |
|-----|-------|-------------------------|
| `.astro` файл | 250 строк | Разделить на компоненты |
| CSS-файл | 400 строк | Разнести по модулям или scoped-стили |
| `<script>` блок в `.astro` | 80 строк | Вынести в `src/scripts/` |
| Функция | 50 строк | Декомпозировать |
| Параметров функции | 4 | Объединить в объект |
| Вложенность блоков | 3 | Ранний return / extract |

Превышение допустимо только с комментарием `// NOTE: <причина>` в начале файла/функции.

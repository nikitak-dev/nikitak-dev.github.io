...
9. **Refactor-дисциплина:** не больше 1 часа наведения порядка перед изменением поведения; три повторения одного знания — сигнал для выделения.
...
5. **Chesterton's Fence:** перед удалением/рефактором незнакомого кода — понять *почему* он существует. `git log --follow` / `git blame` + контекст использования. Некрасивый код часто решает реальную проблему, которую не видно с первого взгляда
...
- **Normalize symmetries** (Kent Beck) — параллельные структуры должны быть структурно идентичны; любое различие обязано нести смысл. Пример: если `.media-body--file` и `.media-error` делают одно и то же кроме палитры — общий блок + переопределения для конкретного варианта, а не две копии с переменами в середине
- **Explaining variables** для составных условий: `const discretionaryAllowed = s.cellsMoved >= s.penetration` вместо inline-выражения в `if`. Удобна для отладки + объясняет намерение
...
- **Не больше 1 часа наведения порядка перед изменением поведения** (Kent Beck). Если больше — потерян минимальный набор структурных правок и скатываемся в бесконечный рефакторинг. Остановиться, сделать commit что есть, вернуться к задаче
- **YAGNI («Say No»):** не добавлять спекулятивные фичи/абстракции. Три похожих случая > преждевременная абстракция. Три подтверждённых повторения одного знания — сигнал для выделения. Дублирование кода < неправильная абстракция
- **Документировать инварианты** в коде, когда они неочевидны: `// Invariant: segments[-1] is always the head`, `// Invariant: occupancy is rebuilt each tick`. Инвариант — компактное описание поведения во всех путях исполнения, снижает когнитивную нагрузку при правках
...
- **Глубина message = значимости изменения.** Мелкое (опечатка, пробелы, пропущенный import) — одна строка. Существенное (новая фича, нетривиальный рефактор, breaking) — тело с обоснованием/контекстом. Тело только если описание не помещается в одну строку
- **File moves (`git mv`) — в отдельном коммите**, правки — в follow-up. Иначе git не опознаёт rename (порог по содержимому не срабатывает), diff раздувается, история `--follow` ломается

**Объединение правок гигиены в текущий коммит:** если опечатка / whitespace / комментарий естественно относится к файлу, который уже меняешь этим коммитом — включать сразу, не дробить на два. Правило «атомарный» не требует механически выделять каждую строчку в отдельный commit.

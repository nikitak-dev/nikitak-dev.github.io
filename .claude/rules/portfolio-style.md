...
10. **Pre-commit hook:** `astro check` + `astro build` + `npx knip --production`. Не отключать — чинить/ставить.
...
Pre-commit hook (`.git/hooks/pre-commit`) автоматически выполняет шаги 1 и 4: `astro check` + `astro build` + `npx knip --production` (dead exports). Если hook не установлен — установить, а не пропускать проверки.
...
4. `npm run check` + `npx knip --production` — 0 ошибок, 0 dead exports
...
**Постоянный gate в CI:** `.github/workflows/pr-check.yml` прогоняет `npx knip --production` — ловит dead TS-exports до merge. Если PR вводит неиспользуемый export — рефакторить или удалять, а не игнорировать предупреждение.

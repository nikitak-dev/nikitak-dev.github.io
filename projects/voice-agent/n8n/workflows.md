...
| `end_of_call` | webhook | active | Receives Vapi's end-of-call report. **Authenticated via `Header Auth` (Bearer in `Authorization` header)** — rejected at HTTP layer if missing/wrong. Looks up customer by `customer.number` (E.164); `extract_call_data` Code node parses transcript, tool-call aggregates, costs, structured `analysisPlan` outputs (matched by `name` field, with drift detection); Postgres `upsert` into `calls` matching on `vapi_call_id` (idempotent on retries); `verify_persisted` Code node asserts the upserted row carries `id` + `vapi_call_id`; **`fetch_assistant_first_message`** HTTP node pulls live `firstMessage` from Vapi `GET /assistant/{id}` (Bearer auth via `Vapi API Bearer Token` credential) so the disclosure text recorded for audit reflects exactly what was spoken to this caller; **`record_consent`** Postgres `upsert` writes a TCPA audit row into `consent_log` matching on `(vapi_call_id, consent_type)`; respond `200`; fire-and-forget `archive_recording` sub-workflow runs in parallel. |
...
The current Postgres schema (Supabase) is documented in [`../db/README.md`](../db/README.md). Four tables: `customers`, `calls`, `appointments`, `consent_log`. Highlights:
...
- **Supabase Postgres:** `client_lookup`, `create_client`, `book_event`, `event_lookup`, `update_event`, `delete_event`, `end_of_call`. One database, four tables (`customers`, `calls`, `appointments`, `consent_log`).
...
- **Postgres credential** — single `supabase-voice_agent` credential (Session Pooler, port 5432) shared by all Postgres nodes. Storage upload uses a separate `supabase-service_role` Bearer Auth credential (the Supabase project's service_role JWT — bypasses RLS).
- **Webhook auth credential** — `end_of_call` webhook node uses an n8n `Header Auth` credential (`Vapi Webhook Auth`) that stores `Authorization: Bearer <secret>`. The matching `Authorization: Bearer <secret>` lives on the Vapi side under `assistant.server.headers`. Rotation is a two-place edit (n8n credential value + Vapi assistant header); no path retag, no redeploy. → [ADR-007](../adrs/007-webhook-auth-bearer-header.md)
- **Vapi API Bearer Token** — separate Bearer Auth credential used by `fetch_assistant_first_message` HTTP node inside `end_of_call`. Holds the Vapi private API key (Vapi Dashboard → Profile → API Keys → private). Lets the workflow pull `assistant.firstMessage` per call so `consent_log.disclosure_text` always reflects the exact greeting that was spoken (instead of a hardcoded copy that would drift on prompt edits).

## Known limitations and improvement candidates

- **MCP trigger path** (`n8n_orchestrator`) is unauthenticated at the URL level — security relies on the path acting as a shared secret plus the Bearer credential check on each MCP request (Vapi-side Custom Credential). Rotate the path if it leaks. The `end_of_call` REST webhook **is** Header-Auth-protected — see [ADR-007](../adrs/007-webhook-auth-bearer-header.md).
- **Saved execution data carries the Bearer header in clear text** (`body.headers.authorization`). Visible to anyone with n8n access. Acceptable for the current single-user dev setup; revisit with `EXECUTIONS_DATA_SAVE_ON_SUCCESS=none` (or per-workflow Save settings) before binding a public phone number — see ADR-007 trade-offs.
...
- **Service-area address verification was removed from the booking flow** — LLM-based distance matching against the KB city list produced flaky results (false negatives on in-radius addresses not literally listed; false positives on ambiguous city names). Sophie now collects the address without verifying it; operations team confirms post-booking. Replacement (geocoding sub-workflow) documented as a growth point in [`architecture.md` Possible Improvements](../architecture.md#possible-improvements).

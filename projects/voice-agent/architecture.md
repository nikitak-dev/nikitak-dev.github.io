...
- **Emergency vs identification gates.** The tree/storm emergency rule in the prompt's `Determine Intent` section routes to the emergency phone (with the number stated twice for clarity) and ends the call without any caller identification — safe because Sophie discloses no caller-specific data and makes no CRM tool calls along that path. Other urgencies (e.g., "I need this ASAP" but not tree/storm) still go through the normal identification + secondary-verification path; there is no general "urgency bypasses verification" rule, by design — the friction cost of verification is low for legitimate callers, and bypassing it opens an impersonation vector ("it's urgent, give me my appointment details").
- **First Message vs system-prompt greeting** — single source of truth is the Vapi assistant's `First Message` field. It carries the opening line including the AI disclosure ("your AI assistant") and the recording notice ("calls may be recorded for quality and training"). Vapi proxies this to TTS instantly at call connect — no LLM round-trip, lower TTFT for the caller. The system prompt's `Immediate Phone Lookup` section explicitly tells Sophie that she does NOT greet herself; she only initiates the parallel `n8n_orchestrator` phone lookup so the result is ready by the time the caller finishes responding to the greeting. Duplicating the greeting string in both places would invite drift and double-greeting bugs.
...
Capabilities within reach but not currently implemented — documented here so the gap between "what production-grade looks like" and "what this MVP does today" is explicit. Mirrors the discipline of the [`multimodal-rag` Possible Improvements section](../multimodal-rag/README.md) — half-baked logic is removed from the runtime so the agent never gets confused; the missing piece is captured here for future implementation.

- **Service-area address verification via geocoding.** Today the booking flow does NOT actively verify that a caller's property address falls within the 35-mile service radius — Sophie collects the address as-is and the operations team confirms post-booking. The earlier prompt-driven approach (Sophie called `search_knowledge_base` for the city list and judged geographically against it) was removed because LLM distance-matching against an unstructured city list produces flaky results: false negatives on in-radius addresses not literally listed (e.g., Apollo Beach FL — inside the 35-mile circle but absent from the KB enumeration), and false positives on ambiguous city names (e.g., "Sun City" — could be Tampa-area Sun City Center or Sun City California, 2,500 miles away). Production fix: a dedicated `verify_address_in_service_area` n8n sub-workflow that calls Google Maps Distance Matrix API with the property address and the business HQ (`Saint Petersburg, FL`), returns boolean `in_radius` based on a 35-mile threshold. The current prompt fallback — "for ambiguous service-area informational queries, offer a callback" — covers the gap until the geocoding sub-workflow lands.

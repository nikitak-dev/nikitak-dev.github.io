...
|---|---|---|
| `plans/` | ✓ | Implementation plans, kept as ADRs. Each completed plan carries a `**Status:** ✅ Implemented` marker in its header. |
...
Astro builds only `src/` + `public/`, so nothing here reaches `dist/`. `README.md`, `plans/` and `eval/` are tracked (small text + Python, ~40KB); bulky `test_data/` is gitignored at the repo root — see [.gitignore](../../.gitignore).
...
**Про историю чата.** История разговора живёт только внутри этой вкладки браузера. Refresh (Перезагрузка страницы) — сохраняет её. Стирают: CLR и закрытие вкладки. Разные вкладки историю не делят — каждая новая начинает с нуля.
...
| Chat LLM | `anthropic/claude-sonnet-4` через OpenRouter | Генерация grounded-ответов (на основе найденных документов) из извлечённого контекста |
| Orchestration | n8n (self-hosted) | 3 workflows — ingestion, chat, error handler |
| Frontend | Astro 6 + vanilla TypeScript | Статическая страница, состояние чата на вкладку |

### // ARCHITECTURE

Три n8n workflows работают вместе. `ingestion` (24 ноды) следит за Google Drive и кормит Pinecone. `chat` (9 нод) отвечает на вопросы пользователя. `error_handler` (2 ноды) ловит необработанные сбои в любом из первых двух.
...
- `extract_text` / `prepare_image` / `prepare_pdf` / `prepare_audio` / `prepare_video` — подготовка по модальности (text — sentence-aware chunking; image/pdf — base64-кодирование; audio/video — base64 + duration probe с проверкой cap)
...
- `notify_skipped` — Discord-алерт когда файл отклонён по cap (audio > 180с, video > 120с); независим от `error_handler` — это graceful skip, а не unhandled error
...
**// ERROR HANDLER** — принимает payload упавших выполнений от обоих workflows (`ingestion` и `chat`).
...
Архитектура выше описывает «что делает каждая нода». Этот раздел — про «почему именно так»: архитектурные решения, которые не очевидны из списка нод.
...
- **LLM-driven intent + modality classification.** Не каждый ввод — это содержательный вопрос. `hi`, `?` и «сколько у вас видео?» требуют разной обработки, а «а что со вторым?» без контекста вообще бессмысленно. Regex-эвристики не масштабируются на EN / RU / UA. Поэтому `rewrite_question` (Gemini 2.0 Flash, temperature 0) делает три задачи одним вызовом: классифицирует вход как `greeting | pure_meta | content`, определяет конкретную модальность, если она в вопросе, и переписывает местоимения на основе recent history в самодостаточный запрос для retrieval. Greetings всё равно проходят через embed / query / rerank, но `build_context` отбрасывает результаты и возвращает пустые `sources` — UI не показывает source-плитки для «hi»; `pure_meta`-запросы идут через pre-retrieval фильтр ниже; `content`-запросы сохраняют cross-modal совпадения.
- **Pre-retrieval metadata filter для `pure_meta`-запросов.** Мета-вопросы про сам корпус («есть ли у вас видео», «какие PDF», «сколько аудио») — структурные: они про то, что *существует*, а не про релевантность контента. Vector similarity и cross-encoder rerank заточены под scoring контента, а не перечисление файлов; если спросить «сколько видео», reranker прогонит короткие video-captions против абстрактного count-вопроса и уверенно задвинет их ниже длинных текстовых пассажей, к теме отношения не имеющих. Поэтому когда классификатор помечает вопрос как `pure_meta` с конкретной `modality`, к запросу Pinecone прикладывается metadata-фильтр (`fileType = modality`) ещё до retrieval. Top-K приходит уже суженным до нужного типа, rerank ранжирует внутри чистого пула, и ответ корректен независимо от того, как cross-encoder относится к длине captions. Паттерн self-query retrieval: LLM-классификатор сам выводит filter predicate (условие фильтра вида `fileType = video`), а vector DB применяет его структурно — до векторного поиска.
- **Modality boost — механизм спасения коротких captions.** Reranker, обученный на корпусах с длинным текстом, склонен занижать оценку коротких описаний (изображения, короткого аудио, одностраничного PDF) по сравнению с длинными текстовыми пассажами — даже когда короткий вариант — это именно то, что нужно. Boost-loop в `build_context` возвращает items нужной модальности обратно в выдачу, независимо от того, как cross-encoder их ранжировал. Шаг работает для обоих intent, но по разным причинам. Для `content`-запросов, где рядом с темой упомянута модальность («покажи картинку AES»), boost страхует cross-modal совпадения от низких rerank-score. Для `pure_meta`-запросов boost ещё критичнее: pre-retrieval сужает top-K до одной модальности, но reranker регулярно роняет ВСЕ items под floor, когда вопрос абстрактный («есть ли у вас видео?») — без boost запрос-перечисление видео вернул бы ноль items. Strict-modality filter в конце (только для `pure_meta`) гарантирует, что финальный список остаётся type-pure даже после boost.
- **Двухуровневый score floor: контекст LLM vs media-плитки UI.** Источник с низкой релевантностью всё ещё может быть полезен LLM как фоновая информация, но в UI как медиа-плитка он выглядит как мусор или неправильный ответ. Поэтому rerank floor разделён по назначению: items ниже 0.001 отбрасываются полностью (true noise); items от 0.001 до 0.5 идут в LLM как контекст, но не отображаются как плитки; items 0.5 и выше попадают и в текстовый контекст, и в media-плитки. Порог 0.5 — индустриальный стандарт релевантности reranker (Pinecone, NVIDIA). Исключение: когда запрос пользователя явно упоминает тип медиа (modality-подсказка от классификатора), хотя бы одна плитка этого типа должна попасть в UI даже при низкой оценке reranker'а — короткие captions системно занижаются, но пользователю, явно запросившему конкретный тип, ответ должен показаться плиткой, а не только текстом. Два случая: `pure_meta`-запросы пропускают все items этого типа (pre-filter Pinecone уже ограничил retrieval нужным `fileType`); `content`-запросы с modality-подсказкой пропускают только лучший по score item этого типа (остальные должны пройти порог сами, чтобы случайный шум того же типа не отображался).
...
- **Sentence-aware chunking с overlap.** Наивный fixed-size chunking режет текст посреди предложения и расщепляет ключевые термины через границу фрагмента — запрос про `AES-256` может промахнуться мимо фрагмента, где есть только `...AES-` или `256 is a...`. Поэтому текстовые файлы режутся по границам предложений, с overlap в 50 токенов между соседними фрагментами. Предложения остаются целыми, а концепции, растянутые на два предложения, всё ещё можно найти, потому что оба фрагмента содержат соединительную фразу.
- **Pre-send duration probe + skip-routing.** Без валидации oversized-файлы падают downstream, когда Gemini Embedding 2 отклоняет их за превышение своих 180с audio / 120с video caps — обобщённый `NodeApiError` в `error_handler`, без сигнала, что причина именно в длительности. Поэтому duration пробится перед отправкой: `prepare_audio` читает WAV через RIFF chunks, MP3 через ID3v2-skip + frame-header bitrate (CBR-приближение, ±5% на VBR); `prepare_video` использует Drive `videoMediaMetadata.durationMillis` как primary, MP4/MOV `mvhd` → `trak.mdia.mdhd` binary parse как fallback для не-Drive источников. Oversized items получают флаг `_oversized: true` и пропускают base64-кодирование. `filter_audio_size` / `filter_video_size` IF-ноды разводят каждую ветку: oversized идёт в `notify_skipped` (Discord-алерт: workflow name, file, scenario, probed duration vs cap, UTC-timestamp, Drive link); valid идёт в caption. Valid файлы из того же batch не страдают — skip per-item, не валит workflow целиком. `prepare_upsert` отфильтровывает `_oversized` items до zip с vectors — в Pinecone попадают только валидные embeddings.
...
| Embedding multipart parts | per-call caps Gemini Embedding 2: ≤6 изображений, ≤1 PDF-файл (сам до 6 страниц), ≤1 audio, ≤1 video на вызов `embedContent`; общий ≤8192 input tokens. Workflow шлёт 2 элемента в `content.parts`: 1 binary + 1 caption-text. См. [Possible Improvements](#improvements) |
...
| Rerank | После cross-encoder rerank: **top-N = 5** идёт в prompt LLM (достаточно для main topic + смежных деталей, без раздувания context); **двухуровневый score floor** — **0.001** для контекста LLM (отсекает true noise) и **0.5** для media-плиток в UI (индустриальный стандарт reranker); **modality boost cap 6** — потолок финальной выдачи источников после работы boost. Boost дополняет top-5 от rerank items нужного `fileType`, но останавливается, когда общее число достигает 6 — это удерживает выдачу от неограниченного роста, при этом гарантируя представление запрошенной модальности |
...
- **Per-chunk embedding для PDF.** Сейчас workflow шлёт весь PDF одним `inlineData` blob и хранит один вектор на файл. Разбиение длинных PDF на чанки по 6 страниц (per-call PDF cap Gemini Embedding 2) и embedding каждого чанка отдельно дало бы более гранулярный retrieval — страничные диапазоны становились бы адресуемыми по отдельности, и запрос про конкретный раздел попадал бы на нужный чанк, а не на документ целиком.
...
Система прогоняется через end-to-end набор из 39 тестовых кейсов, покрывающий **14 классов failure mode** — и проверяющий, что система не падает ни на одном. 14 классов делятся на **6 проектно-специфичных** (нет аналога в Barnett et al. 2024 *Seven Failure Points of RAG*): language drift, prompt injection, multi-turn coherence, modality disambiguation, typo tolerance и corpus-meta queries (вопросы про то, что *есть* в корпусе, vs про то, что *написано в файлах*). Оставшиеся **8 покрывают все 7 Barnett FP** по принципу N-to-1 (несколько наших классов могут целиться в один FP — разные углы одного failure mode): FP1 missing content — hallucination guards и negative refusals; FP2 missed top-ranked + FP3 not-in-context — cross-topic источники, на которых retrieval/consolidation путаются; FP4 not extracted — faithful extraction из context; FP5 wrong format — неявно через completeness-тесты вроде «list every SQL JOIN type»; FP6 incorrect specificity — fragment и generic запросы; FP7 incomplete — completeness ответов. Плюс общая edge-case robustness через все FPs.

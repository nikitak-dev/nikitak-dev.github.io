...
Система прогоняется через end-to-end набор из 39 тестовых кейсов, покрывающий **14 классов failure mode** — и проверяющий, что система не падает ни на одном. 14 классов расширяют 7 базовых из Barnett et al. 2024 (*Seven Failure Points of RAG*) проектно-специфичными добавлениями: language drift, prompt injection, multi-turn coherence, modality disambiguation и др. Среди конкретных failure mode'ов: галлюцинировать за пределами корпуса, отвечать не на том языке, путать похожие источники, следовать враждебным инструкциям, подсунутым внутри вопроса или истории, смешивать вопрос о том, что *есть* в корпусе, с вопросом о том, что *в нём написано*, терять местоимения между репликами и ломаться на опечатках.

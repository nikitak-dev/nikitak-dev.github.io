...
| Rerank | После cross-encoder rerank: **top-N = 5** идёт в prompt LLM (достаточно для main topic + смежных деталей, без раздувания context); **score floor 0.001** отсекает true noise (items ниже этого порога редко дают полезный сигнал); **modality boost cap 6** — modality boost может довести result set до 6 items нужного типа, чтобы запросы вроде «перечисли все видео» получили покрытие без переполнения prompt |
